...
class EventPhase:
    ...
    phase_code: str = field(
        metadata={"max_length": 30, "description": "阶段代码 (NOT NULL)"}
    )
    display_name: str = field(
        metadata={"max_length": 50, "description": "显示名称 (NOT NULL)"}
    )
    ...
    target_score: Optional[int] = field(
        default=None, metadata={"description": "目标分数"}
    )
    is_final_phase: bool = field(
        default=False, metadata={"description": "是否为决赛阶段"}
    )
