...
class Rule(VersionedModel):
    ...
    # 外键 (FKs)
    elimination_type_id: UUID = field(metadata={"foreign_key": "Elimination_Type", "description": "淘汰赛类型 (如单败淘汰)"})
    final_ranking_type_id: UUID = field(metadata={"foreign_key": "Ranking_Type", "description": "名次决出方式 (如是否有铜牌赛)"})
    ...
    # 可选字段 (Nullable)
    match_format: Optional[str] = field(default=None, metadata={"max_length": 50, "description": "单场比赛格式 (如 3x3 min / 1x5 min)"})
    pool_size: Optional[int] = field(default=None, metadata={"db_constraint": "CHECK (>=3)", "description": "小组赛每组人数"})
    ...
    group_qualification_ratio: Optional[Decimal] = field(
        default=None, metadata={"decimal_precision": (5, 4), "description": "晋级比例（备用）"}
    )
