...
class PoolBout:
    ...
    fencer_a_id: UUID = field(
        metadata={"foreign_key": "Fencer", "description": "运动员A"}
    )
    fencer_b_id: UUID = field(
        metadata={"foreign_key": "Fencer", "description": "运动员B"}
    )
    ...
    actual_start_time: Optional[datetime] = field(
        default=None, metadata={"description": "实际开始时间"}
    )
    actual_end_time: Optional[datetime] = field(
        default=None, metadata={"description": "实际结束时间"}
    )
