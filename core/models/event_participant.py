...
class EventParticipant:
    ...
    seed_value: Optional[float] = field(
        default=None, metadata={"description": "种子分值"}
    )
    is_confirmed: bool = field(default=True, metadata={"description": "是否确认参赛"})
    registration_time: Optional[datetime] = field(
        default=None, metadata={"description": "报名时间"}
    )
    notes: Optional[str] = field(default=None, metadata={"description": "备注"})
    created_at: datetime = field(
        default_factory=datetime.now, metadata={"description": "创建时间"}
    )
    updated_at: datetime = field(
        default_factory=datetime.now, metadata={"description": "更新时间"}
    )
