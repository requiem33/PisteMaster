...
class EventSeed:
    ...
    seed_type_id: UUID = field(
        metadata={"foreign_key": "Seed_Type", "description": "种子类型"}
    )
    seed_rank: int = field(metadata={"description": "种子排名 (NOT NULL)"})

    seed_value: Optional[Decimal] = field(
        default=None, metadata={"decimal_precision": (10, 2), "description": "种子分值"}
    )
