...
class TeamMatch:
    ...
    match_code: str = field(
        metadata={"max_length": 20, "description": "比赛编号 (NOT NULL)"}
    )
    ...
    team_a_id: Optional[UUID] = field(
        default=None, metadata={"foreign_key": "Team", "description": "队伍A"}
    )
    team_b_id: Optional[UUID] = field(
        default=None, metadata={"foreign_key": "Team", "description": "队伍B"}
    )
    winner_team_id: Optional[UUID] = field(
        default=None, metadata={"foreign_key": "Team", "description": "获胜队伍"}
    )
