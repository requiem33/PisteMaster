...
> **Note**: Features marked as "Post-MVP" are planned for future releases. See `docs/MVP_BUILD_PLAN.md` for current scope.
...
- **A. Electron 应用外壳 (`PisteMaster.exe`)**
    - **技术栈**: **Electron + Vue 3** (MVP: Reuse existing Vue frontend)
    - **职责**:
        - **UI 界面 (Renderer Process)**: 运行 Vue 应用，负责所有用户交互。
        - **后台协调器 (Main Process)**:
            - **进程管理器**: 在后台静默启动和管理 `backend.exe` 的生命周期。
            - **网络协调器** (Post-MVP): 使用 **etcd** 客户端进行主节点选举和服务发现。
            - **命令代理** (Post-MVP): 拦截 UI 发出的"写"请求，如果自身是从节点，则将请求转发给主节点。
            - **事件客户端** (Post-MVP): 作为 WebSocket 客户端，接收主节点广播的"事件"并触发本地数据的更新。
...
> **Note**: The original design mentioned PySide6 for desktop UI, but MVP will use Electron to reuse the existing Vue frontend. See `docs/MVP_BUILD_PLAN.md` for implementation details.
...
> **Note**: The detailed MVP build and deployment plan has been moved to a dedicated document.
> 
> **See: `docs/MVP_BUILD_PLAN.md`** for:
> - Phase 1: WSL Docker Deployment (PostgreSQL)
> - Phase 2: Electron Desktop App (Windows)
> - Phase 3: Export & Print Features
> - Phase 4: Offline Sync

The following is the original roadmap for reference:

**Phase 1: 后端准备与容器化**

- [x] **1.1. 完善 Django 模型**: 根据 `database_schema.md`，最终确认 `apps/core/models.py` 中的所有模型定义。
- [x] **1.2. 适配双数据库**:
    - [x] 修改 `settings.py`，使其能通过环境变量在 PostgreSQL（云端）和 SQLite（桌面端）之间无缝切换。
    - [x] 确认 Python 环境中的 `sqlite3` 模块版本 >= 3.9.0，以保证 `JSONField` 的兼容性。
- [x] **1.3. 完善核心 API**: 使用 DRF 确保所有核心业务（创建/获取赛事、更新比分、保存排名）都有稳定可靠的 API 接口。
...
- [x] **2.1. 重构 `DataManager.ts`**:
    - [x] 将所有 `IndexedDBService` 的直接调用，替换为 `fetch` 或 `axios` HTTP 请求，目标地址为 `http://localhost:8000`。
    - [x] 确保所有 `GET`、`POST`、`PUT` 请求都能与 `backend.exe` 提供的 API 正常通信。
- [ ] **2.2. 实现"乐观 UI"模式**:
