...
class EventParticipantViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        # 不允许更新event和fencer
        if (
            "event" in serializer.validated_data
            or "fencer" in serializer.validated_data
        ):
            return Response(
                {"detail": "不能修改事件或运动员"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def bulk_register(self, request):
        ...
        if not event_id:
            return Response(
                {"detail": "event_id is required"}, status=status.HTTP_400_BAD_REQUEST
            )

        try:
            participant_service = EventParticipantService()
            successful, failed = participant_service.bulk_register_fencers(
                event_id, fencer_ids
            )
    ...
    def confirm_participation(self, request, pk=None):
        ...
        try:
            participant_service = EventParticipantService()
            updated_participant = participant_service.confirm_participation(
                participant.event.id, participant.fencer.id
            )
            ...
            if updated_participant:
                ...
                return Response(
                    {"detail": "确认失败"}, status=status.HTTP_400_BAD_REQUEST
                )
        except EventParticipantService.EventParticipantServiceError as e:
    ...
    def unconfirm_participation(self, request, pk=None):
        ...
        try:
            participant_service = EventParticipantService()
            updated_participant = participant_service.unconfirm_participation(
                participant.event.id, participant.fencer.id
            )
            ...
            if updated_participant:
                ...
                return Response(
                    {"detail": "取消确认失败"}, status=status.HTTP_400_BAD_REQUEST
                )
        except EventParticipantService.EventParticipantServiceError as e:
            return Response({"detail": e.message}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def update_seeds(self, request):
        ...
        try:
            participant_service = EventParticipantService()
            updated_participants = participant_service.update_seed_ranking(
                event_id, seed_updates
            )

            updated_participant_ids = [p.id for p in updated_participants]
            django_participants = DjangoEventParticipant.objects.filter(
                id__in=updated_participant_ids
            )
            output_serializer = EventParticipantSerializer(
                django_participants, many=True
            )
    ...
    def by_event(self, request, event_id=None):
        ...
        confirmed_only = (
            request.query_params.get("confirmed_only", "true").lower() == "true"
        )

        participant_service = EventParticipantService()
        participants = participant_service.get_event_participants(
            event_uuid, confirmed_only
        )
