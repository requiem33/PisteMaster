...
class RuleViewSet(viewsets.GenericViewSet):
    ...
    def list(self, request):
        ...
        if search:
            ...
            rules = [
                r
                for r in rules
                if search_lower in r.rule_name.lower()
        ...
        if rules and hasattr(rules[0], order_field):
            rules = sorted(
                rules, key=lambda x: getattr(x, order_field) or "", reverse=reverse
            )
    ...
    def retrieve(self, request, pk=None):
        try:
            ...
            return Response(
                {"detail": "Invalid rule ID"}, status=status.HTTP_400_BAD_REQUEST
            )

        rule = self.service.get_rule_by_id(rule_id)
        if not rule:
            return Response(
                {"detail": "Rule not found"}, status=status.HTTP_404_NOT_FOUND
            )
