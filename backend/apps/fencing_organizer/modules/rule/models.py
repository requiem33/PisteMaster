...
class DjangoRule(models.Model):
    ...
    elimination_type = models.ForeignKey(
        DjangoEliminationType, on_delete=models.PROTECT, db_column="elimination_type_id", related_name="rules", verbose_name="淘汰赛类型"
    )

    final_ranking_type = models.ForeignKey(
        DjangoRankingType, on_delete=models.PROTECT, db_column="final_ranking_type_id", related_name="rules", verbose_name="名次决出方式"
    )

    # 可选字段
    match_format = models.CharField(max_length=50, null=True, blank=True, verbose_name="单场比赛格式")

    pool_size = models.IntegerField(null=True, blank=True, verbose_name="小组赛每组人数")

    match_duration = models.IntegerField(null=True, blank=True, verbose_name="单局时长（秒）")

    match_score_pool = models.IntegerField(null=True, blank=True, verbose_name="小组赛目标分数")

    match_score_elimination = models.IntegerField(null=True, blank=True, verbose_name="淘汰赛目标分数")

    group_qualification_ratio = models.DecimalField(max_digits=5, decimal_places=4, null=True, blank=True, verbose_name="晋级比例")
    ...
    is_preset = models.BooleanField(default=False, verbose_name="是否为预设规则")
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(check=models.Q(pool_size__gte=3) | models.Q(pool_size__isnull=True), name="chk_rule_pool_size"),
            models.CheckConstraint(
                check=models.Q(group_qualification_ratio__gte=0) & models.Q(group_qualification_ratio__lte=1)
