...
class DjangoPisteRepository:
    ...
    def get_by_tournament(self, tournament_id: UUID) -> List[Piste]:
        """获取指定赛事的剑道"""
        django_pistes = DjangoPiste.objects.select_related("tournament").filter(tournament_id=tournament_id).order_by("piste_number")
    ...
    def save(self, piste: Piste) -> Piste:
        ...
        django_piste, created = DjangoPiste.objects.update_or_create(id=piste.id, defaults=orm_data)
