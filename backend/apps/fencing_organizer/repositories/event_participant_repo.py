...
from backend.apps.fencing_organizer.mappers.event_participant_mapper import (
    EventParticipantMapper,
)
from backend.apps.fencing_organizer.modules.event_participant.models import (
    DjangoEventParticipant,
)
from core.interfaces.event_participant_repository import (
    EventParticipantRepositoryInterface,
)
...
class DjangoEventParticipantRepository(EventParticipantRepositoryInterface):
    ...
    def get_participants_by_event(
        ...
        query = query.select_related("event", "fencer").order_by(
            "seed_rank", "fencer__last_name", "fencer__first_name"
        )
    ...
    def get_participants_by_fencer(self, fencer_id: UUID) -> List[EventParticipant]:
        """获取指定运动员的参赛记录"""
        participants = (
            DjangoEventParticipant.objects.filter(
                fencer_id=fencer_id, is_confirmed=True
            )
    ...
    def save_participant(self, participant: EventParticipant) -> EventParticipant:
        ...
        django_participant, created = DjangoEventParticipant.objects.update_or_create(
            id=participant.id, defaults=orm_data
        )
            ...
            for fencer_id, seed_rank in seed_updates:
                try:
                    ...
                    updated_participants.append(
                        EventParticipantMapper.to_domain(participant)
                    )
                except DjangoEventParticipant.DoesNotExist:
                    continue
    ...
    def get_event_stats(self, event_id: UUID) -> Dict[str, Any]:
        ...
        stats["by_weapon"] = [
            ...
            if stat["fencer__primary_weapon"]
        ]
    ...
    def get_participant_count_by_event(self, event_id: UUID) -> int:
        ...
        return DjangoEventParticipant.objects.filter(
            event_id=event_id, is_confirmed=True
        ).count()
    ...
    def get_top_seeded_fencers(
        ...
        participants = DjangoEventParticipant.objects.filter(
            event_id=event_id, is_confirmed=True, seed_rank__isnull=False
