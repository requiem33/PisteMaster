...
class DjangoEventParticipantRepository(EventParticipantRepositoryInterface):
    ...
    def get_participant_by_id(self, participant_id: UUID) -> Optional[EventParticipant]:
        """根据ID获取参与者"""
        try:
            django_participant = DjangoEventParticipant.objects.select_related("event", "fencer").get(pk=participant_id)
    ...
    def get_participants_by_fencer(self, fencer_id: UUID) -> List[EventParticipant]:
        """获取指定运动员的参赛记录"""
        participants = (
            DjangoEventParticipant.objects.filter(fencer_id=fencer_id, is_confirmed=True)
            .select_related("event", "fencer")
    ...
    def get_participant(self, event_id: UUID, fencer_id: UUID) -> Optional[EventParticipant]:
        """获取指定项目和运动员的参与记录"""
        try:
            django_participant = DjangoEventParticipant.objects.select_related("event", "fencer").get(
                event_id=event_id, fencer_id=fencer_id
            )
            return EventParticipantMapper.to_domain(django_participant)
        except DjangoEventParticipant.DoesNotExist:
            return None
    ...
    def save_participant(self, participant: EventParticipant) -> EventParticipant:
        ...
        django_participant, created = DjangoEventParticipant.objects.update_or_create(id=participant.id, defaults=orm_data)
    ...
    def remove_participant(self, event_id: UUID, fencer_id: UUID) -> bool:
        """移除参与者"""
        try:
            count, _ = DjangoEventParticipant.objects.filter(event_id=event_id, fencer_id=fencer_id).delete()
            return count > 0
        except Exception:
            return False

    def update_seed_ranks(self, event_id: UUID, seed_updates: List[Tuple[UUID, int]]) -> List[EventParticipant]:
        """批量更新种子排名"""
        ...
        with transaction.atomic():
            for fencer_id, seed_rank in seed_updates:
                try:
                    participant = DjangoEventParticipant.objects.get(event_id=event_id, fencer_id=fencer_id)
    ...
    def get_event_stats(self, event_id: UUID) -> Dict[str, Any]:
        ...
        # 按国家统计
        country_stats = (
            DjangoEventParticipant.objects.filter(event_id=event_id, is_confirmed=True)
            .values("fencer__country_code")
            .annotate(count=Count("id"))
            .order_by("-count")
        )

        stats["by_country"] = [
            {"country_code": stat["fencer__country_code"], "count": stat["count"]} for stat in country_stats if stat["fencer__country_code"]
        ]

        # 按剑种统计（从fencer获取）
        weapon_stats = (
            DjangoEventParticipant.objects.filter(event_id=event_id, is_confirmed=True, fencer__primary_weapon__isnull=False)
            .values("fencer__primary_weapon")
            .annotate(count=Count("id"))
            .order_by("-count")
        )

        stats["by_weapon"] = [
            {"weapon": stat["fencer__primary_weapon"], "count": stat["count"]} for stat in weapon_stats if stat["fencer__primary_weapon"]
        ]
    ...
    def get_participant_count_by_event(self, event_id: UUID) -> int:
        """获取项目的参与者数量"""
        return DjangoEventParticipant.objects.filter(event_id=event_id, is_confirmed=True).count()

    def get_top_seeded_fencers(self, event_id: UUID, limit: int = 10) -> List[EventParticipant]:
        """获取种子排名最高的参与者"""
        participants = DjangoEventParticipant.objects.filter(event_id=event_id, is_confirmed=True, seed_rank__isnull=False).order_by(
            "seed_rank"
        )[:limit]
    ...
    def get_participants_with_details(self, event_id: UUID) -> List[Dict[str, Any]]:
        """获取参与者详细信息"""
        participants = (
            DjangoEventParticipant.objects.filter(event_id=event_id, is_confirmed=True).select_related("fencer").order_by("seed_rank")
        )
